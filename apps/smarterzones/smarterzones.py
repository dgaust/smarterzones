...
class smarterzones(hass.Hass):
    ...
    def common_zone_manager(self, entity, attribute, old, new, kwargs):
        ...
        max_temp = round(common_zone_target_temp + temperature_offsets[0], 1)
        min_temp = round(common_zone_target_temp - temperature_offsets[1], 1)
        ...
        # If at least one other zone is open and the common zone is open, close the common zone
            ...
            if common_zone_temperature < max_temp and common_zone_temperature > min_temp:
    ...
    def automatically_manage_zone(self, zone):
        ...
        if "dry" in climate_device_state.lower() or "fan" in climate_device_state.lower():
            if not self.override_enabled(zone):
                self.log_info(f"Climate mode is {climate_device_state}, opening {zonename}.")
                self.switch_on(zone)
            else:
                self.log_info(f"Skipping {zonename} as manual override is enabled.")
            return
    ...
    def adjust_target_temperature(self, current_temp, desired_temp, climate_device):
        temp_diff = current_temp - desired_temp
        threshold = 0.5
        min_boost = 0.5
        max_boost = 3.0
        ...
        self.log(f"Adjusting target temperature from {desired_temp} to {new_target_temp} to reach {current_temp} quickly")
